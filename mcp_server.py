    ...
    for d in drawings:
        ...
        if tag in ("ansicht", "schnitt"):
            ...
            if not calib:
                ...
                if calib.get("status") == "transferred":
                    w4_transferred_calibrations.append({
                        "file": f,
                        "source_scene": calib.get("source_scene"),
                        "transfer_kind": calib.get("transfer_kind"),
...
def _compact_plan_mutation_response(data: Any, *, action: str | None = None, max_items: int = 8) -> Any:
    ...
    summary = {
        ...
        "quality_tier": term.get("quality_tier") or current.get("quality_tier"),
        "completion_state": term.get("completion_state") or current.get("completion_state"),
        "review_debt": term.get("review_debt") or current.get("review_debt"),
        "final_qa_summary": term.get("final_qa_summary") or current.get("final_qa_summary"),
...
def _compact_workflow_for_summary(workflow: dict[str, Any], max_blockers: int = 2) -> dict[str, Any]:
    ...
    for phase, data in (workflow.get("phases") or {}).items():
        ...
        if data.get("transferred_calibrations"):
            phases[phase]["transferred_calibration_count"] = len(data["transferred_calibrations"])
    return {
...
_TOOL_PROFILES: dict[str, set[str]] = {
    "inventory": {
        ...
        "split_scene", "get_scene_view", "read_scene_region",
        "zoom_read_scene_region", "get_scene_meta",
    ...
    "floorplan": {
        ...
        "record_dimension_chain_review",
        "add_scene_plan_evidence", "set_scene_plan_task_state",
        "evaluate_scene_plan_gates",
        "get_scene_view", "read_scene_region", "zoom_read_scene_region",
        ...
        "review_wall_centerline_between_rails", "close_wall_graph",
